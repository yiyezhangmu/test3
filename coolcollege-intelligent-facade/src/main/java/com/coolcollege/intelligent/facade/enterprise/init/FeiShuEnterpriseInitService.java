...
import com.coolstore.base.enums.FSEventTypeEnum;
 ...
 * @FileName: FeiShuEnterpriseInitService
 * @Description:
 * @date 2022-11-17 17:34
 */
@Slf4j
@Service
public class FeiShuEnterpriseInitService extends EnterpriseInitBaseService{
    ...
    public void enterpriseInit(String corpId, String eid, String appType, String dbName, String openUserId) {
        try {
            ...
            List<String> adminUserList = enterpriseInitConfigApiService.getAdminUserList(corpId, appType);
            //添加ai用户
            ...
            if(CollectionUtils.isEmpty(sysDepartmentDTOS)){
                ...
                root.setId(Constants.ROOT_DEPT_ID_STR);
            ...
            //初始化用户
            initUser(sysDepartmentDTOS,corpId,eid,appType,dbName, adminUserList, authInfo, handlerUserIds, false);
    ...
    public void enterpriseInitUser(String corpId, String eid, String appType, String dbName, Boolean isScopeChange) {
        try {
            DataSourceHelper.changeToSpecificDataSource(dbName);
            List<String> adminList = enterpriseInitConfigApiService.getAdminUserList(corpId, appType);
            List<SysDepartmentDTO> sysDepartmentDTOS = initDept(corpId, eid, appType, dbName);
...
//            deleteUser(eid, handlerUserIds);
    ...
    public void onlySyncUser(String corpId, String eid, String appType, String dbName) {
        try {
            DataSourceHelper.changeToSpecificDataSource(dbName);
            List<String> adminList = enterpriseInitConfigApiService.getAdminUserList(corpId, appType);
            List<SysDepartmentDTO> sysDepartmentDTOS = getAllDept(corpId, appType);
            //记录此次处理的用户的id， 用于做用户无用用户的删除
            Set<String> handlerUserIds = new HashSet<>();
            //获取开通授权信息
            AuthInfoDTO authInfo = enterpriseInitConfigApiService.getAuthInfo(corpId, appType);
    ...
    public List<SysDepartmentDTO> getAllDept(String corpId, String appType) {
    ...
    public void initRootRegion(List<SysDepartmentDTO> sysDepartmentDTOS, String eid, String dbName) {
        ...
        try {
            Optional<SysDepartmentDTO> first = sysDepartmentDTOS.stream()
                    .filter(s -> Objects.equals(SyncConfig.ROOT_DEPT_ID_STR, s.getId())).findFirst();
    ...
    private void userAddOrUpdate(String corpId, String userId, String appType, String enterpriseId, String dbName){
        try {
            DataSourceHelper.changeToSpecificDataSource(dbName);
            //记录此次处理的用户的id，
            Set<String> handlerUserIds = new HashSet<>();
...
//            List<String> deptIdLists = ListUtils.emptyIfNull(departments).stream().map(SysDepartmentDTO::getId).collect(Collectors.toList());
            List<EnterpriseUserRequest> deptUsers = new ArrayList<>();
            EnterpriseUserDTO userDetail = enterpriseInitConfigApiService.getUserDetailByUserId(corpId, userId, appType);
            EnterpriseUserRequest enterpriseUserRequest = new EnterpriseUserRequest();
            enterpriseUserRequest.setEnterpriseUserDO(convertFactory.convertEnterpriseUserDTO2EnterpriseUserDO(userDetail));
            enterpriseUserRequest.setDepartmentLists(userDetail.getDepartmentLists());
            enterpriseUserRequest.setLeaderInDepts(userDetail.getIsLeaderInDepts());
            ...
            dealUsers(deptUsers, enterpriseId, corpId, dbName, deptIdMap, unclassifiedRegionDO.getId(), authInfo, handlerUserIds, false);
        } catch (ApiException e) {
            log.info("远程获取用户详情失败", e);
        }
    }

    private void userDelete(String userId, String enterpriseId, String dbName){
        DataSourceHelper.changeToSpecificDataSource(dbName);
        syncUserFacade.syncDeleteUser(enterpriseId, userId, dbName);
    }

    @Override
    public void userUpdateEvent(AddressBookChangeReqBody param) {
        if(FSEventTypeEnum.USER_CREATED.getValue().equals(param.getEventType()) || FSEventTypeEnum.USER_UPDATED.getValue().equals(param.getEventType())){
            feiShuSyncService.syncFsUser(param.getCorpId(), param.getOpenId(), param.getEnterpriseId(), param.getDbName() ,param.getAppType());
//            userAddOrUpdate(param.getCorpId(), param.getOpenId(), param.getAppType(), param.getEnterpriseId(), param.getDbName());
        }
        if(FSEventTypeEnum.USER_DELETED.getValue().equals(param.getEventType())){
            userDelete(param.getOpenId(), param.getEnterpriseId(), param.getDbName());
        }
    }
}
