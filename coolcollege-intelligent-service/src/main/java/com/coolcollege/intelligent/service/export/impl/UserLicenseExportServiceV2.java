...
import com.coolstore.license.client.dto.LicenseTypeExtendFieldDTO;
...
@Service
@Slf4j
public class UserLicenseExportServiceV2 implements BaseExportService {
    ...
    public ExportServiceEnum getExportServiceEnum() {
        return ExportServiceEnum.USER_LICENSE_EXPORT;
    }
    ...
    public List<?> exportList(String enterpriseId, JSONObject request, int pageSize, int pageNum) {
        ...
        //查寻门店的下的人
        List<String> regionPath = storeAreaDTOS.stream().map(StoreAreaDTO::getRegionPath).collect(Collectors.toList());

        List<EnterpriseUserDO> userDOS = enterpriseUserMapper.selectUserByRegionPaths(enterpriseId, regionPath);
        if (CollectionUtils.isEmpty(userDOS)){
            return vos;
        }
        List<String> userIds = userDOS.stream().map(EnterpriseUserDO::getUserId).collect(Collectors.toList());
        //门店下人员
    ...
    public List<ExcelExportEntity> exportFields(JSONObject request) {
        ...
        beanList.add(buildExcelExportEntity("用户名称", "storeName", orderNum++, null));
        beanList.add(buildExcelExportEntity("所属部门", "regionName", orderNum++, null));
        for (LicenseTypeDTO licenseType : licenseTypes) {
            beanList.add(buildExcelExportEntity("证照状态", "licenseStatus", orderNum++, licenseType.getName()));
            beanList.add(buildExcelExportEntity("过期时间", "endDate", orderNum++, licenseType.getName()));
