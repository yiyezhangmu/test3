...
public class RedisUtilPool {
    ...
    public String hashGet(final String key, final String field, String defaultValue) {
        String result = new Executor<String>(shardedJedisPool) {
     ...
     * @return 给定域的值。当给定域不存在或是给定 key 不存在时，返回 nil 。
     */
    public String hashGetByIdx(final String key, final String field, final int dbIndex) {
        ShardedJedisPool jedisPool = null;
        if (null != shardedJedisPoolMap) {
            jedisPool = shardedJedisPoolMap.get(dbIndex);
        }
     ...
     * @return 给定域的值。当给定域不存在或是给定 key 不存在时，返回 nil 。
     */
    public Map<String, String> hashGetAllByIdx(final String key, final int dbIndex) {
        ShardedJedisPool jedisPool = null;
        if (null != shardedJedisPoolMap) {
            jedisPool = shardedJedisPoolMap.get(dbIndex);
        }
     ...
     * @return
     */
    public long hashDelByIdx(final String key, final int dbIndex, final String... field) {
        ShardedJedisPool jedisPool = null;
        if (null != shardedJedisPoolMap) {
            jedisPool = shardedJedisPoolMap.get(dbIndex);
        }
    ...
    /**
     * 返回有序集 key 中， 介于 start 和 end 之间(默认包括等于 start 或 end )的所有的成员。
     * 有序集成员按 score 值递减(从大到小)的次序排列。
     *
     * @param key key
     * @param start 起始位置 0代表第一位
     * @param end 结束位置  -1代表末尾
...
//                return jedis.zrevrangeByScore(key, max, min);
    ...
    public Boolean isActionAllowed(String actionKey, int period, int maxCount) {
        return new Executor<Boolean>(shardedJedisPool) {
            ...
            Boolean execute() {
                ...
                // 移除时间窗口外的行为
                double v = (nowTs - period * 1000 * 1000 * 1000);
                pipeline.zremrangeByScore(actionKey, 0, v);
                // 查找时间窗口中的行为数量
     ...
     * @param key key
     * @param value 锁value值
     * @return 返回true获取锁成功，返回false获取锁失败
     */
    ...
    /**
     * 获取分布式加锁
     * 注意点  setnx 和 expire 非原子操作
     * @param key key
     * @param value 锁value值
     * @param cacheTime 锁持有时间
     * @return 返回true获取锁成功，返回false获取锁失败
     */
    public Boolean lock(String key, String value, int cacheTime){
        return new Executor<Boolean>(shardedJedisPool) {
            @Override
            Boolean execute() {
                Long setnx = jedis.setnx(key, value);
    ...
    public Boolean unlock(String key, String value){
        return new Executor<Boolean>(shardedJedisPool) {
            ...
            Boolean execute() {
                ...
                if(StringUtils.isNotBlank(keyValue) && keyValue.equals(value)){
